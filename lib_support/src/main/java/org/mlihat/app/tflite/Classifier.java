...
public abstract class Classifier {
  ...
  public static Classifier create(Activity activity, Model model, Device device, int numThreads)
      throws IOException {
    if (model == Model.BUNGA) {
      return new Bunga(activity, device, numThreads);
    } else if (model == Model.BURUNG) {
      return new Burung(activity, device, numThreads);
    } else if (model == Model.KUCING) {
      return new Kucing(activity, device, numThreads);
    } else if (model == Model.ANJING) {
      return new Anjing(activity, device, numThreads);
    }else {
